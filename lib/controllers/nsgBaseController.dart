...
import 'package:nsg_data/controllers/nsg_controller_widget.dart';
import 'package:nsg_data/nsg_data.dart';
import 'package:nsg_data/nsg_data_delete.dart';
import 'nsg_controller_regime.dart';

typedef NsgWidgetBuilder = Widget Function(String status);

class NsgBaseController {
  Type dataType;
  bool requestOnInit;
  ...
  final ValueNotifier<NsgControillerStatus> _currentStatus = ValueNotifier<NsgControillerStatus>(NsgControillerStatus.loading);
  ValueListenable<NsgControillerStatus> get currentStautsListenable => _currentStatus;
  ...
  NsgControillerStatus get currentStatus {
    if (_currentStatus.value == NsgControillerStatus.success) {
      if (masterController != null && masterController!.currentStatus != NsgControillerStatus.success) {
  ...
  set currentStatus(NsgControillerStatus value) => _currentStatus.value = value;
  ...
  void onClose() {
    ...
    currentStatus = NsgControillerStatus.empty;
  }
  ...
  Future _requestItems({NsgDataRequestParams? filter}) async {
    try {
      ...
      // В принципе, если после обновления в выборке нет больше элемента, соответствующего текущеей строке, надо его очищать,
      // но не факт. Например, при динамической подгрузке донных это неверно, по крайней мре, в общем случае
  ...
  void sendNotify({List<NsgUpdateKey>? keys}) {
    ...
    if (useChange && keys == null) {
      ...
      //TODO: обновление по статусу
      // var needRefresh = true; //= currentStatus == status;
      //TODO: TEST IT
      //_currentStatus.value = _currentStatus.value;
      _currentStatus.notifyListeners();
      // if (needRefresh) {
      //   refresh();
      // }
    }
  ...
  NsgControllerWidget obx(NsgWidgetBuilder widget, {Widget Function(String? error)? onError, Widget? onLoading, Widget? onEmpty, List<NsgUpdateKey>? keys}) {
    return NsgControllerWidget(
      widget,
      controller: this,
      onError: onError,
      onEmpty: onEmpty,
  ...
  Future setAndRefreshSelectedItem(NsgDataItem item, List<String>? referenceList) async {
    ...
    try {
      ...
      if (index >= 0) {
        ...
        errorDescription = 'Ошибка NBC-509. Данный объект более недоступен';
        currentStatus = NsgControillerStatus.error;
        sendNotify();
...
//}
  ...
  void update([List<NsgUpdateKey>? keys]) {}

  ///Вызывается при unRegister контроллера в NsgServiceLocator при навигации или прямом вызове
  ///Можно использовать для освобождения ресурсов
  ///Возможно, надо блокировать все методы запроса данных после этого для невозможности дальнейшего использования контроллера
  @mustCallSuper
  void dispose() {
    //TODO: в debugPrint написать собщение об удалении контроллера
  }
}
