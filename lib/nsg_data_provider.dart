...
class NsgDataProvider {
  ...
  //
  // 08.09.2026 (#2000) проверку включили для всех — и на рабочих местах Windows
  // без автообновления корней (Dart сам корни не догружает) десктопные
  // приложения перестали подключаться к серверу: так сломался АРМ Титан 112
  // 0.3.7. Поэтому проверку можно ВЫКЛЮЧИТЬ флагом [verifyCertificates].
  // По умолчанию она ВКЛЮЧЕНА (решение владельца 23.09.2026): безопасное
  // поведение — для всех, а приложение, которому оно мешает (Титан 112 на
  // машинах без корня GlobalSign R46), отключает её у себя явно.
  // Включённая проверка работает с двумя послаблениями ниже.

  ///Проверять TLS-сертификат сервера.
  ///
  ///`true` по умолчанию: сертификат проверяется (защита от подмены — MITM,
  ///перехватчики; разбор NSG-SOFT/futbolista-tasks#2000).
  ///Приложение, на устройствах которого корня сервера может не быть и вшить
  ///его нельзя, отключает проверку при инициализации — осознанно, у себя:
  ///
  ///```dart
  ///NsgDataProvider.verifyCertificates = false;
  ///```
  static bool verifyCertificates = true;

  ///При включённой [verifyCertificates]: хосты, которым разрешено предъявлять непроверяемый сертификат
  ...
  ///При включённой [verifyCertificates]: принимать любой сертификат в ОТЛАДОЧНОЙ сборке.
  ///
  ...
  ///означает «рвём соединение». При выключенной [verifyCertificates] — всегда `true`.
  @visibleForTesting
  static bool shouldAcceptBadCertificate(String host, {bool? isDebugBuild}) {
    if (!verifyCertificates) return true;
  ...
  // Token change notifications for host apps
  ...
  Future<dynamic> baseRequestList({
    ...
    final String? url,
    //сделать настраиваемым параметром
    ...
    try {
      ...
      //сделать генерацию метода запроса GET/POST
      var method2 = 'POST';
  ...
  Future<NsgLoginResponse> requestSocialMethod({
    ...
    try {
      var response = await (baseRequest(
        function: function,
        headers: getAuthorizationHeader(),
        url: '$serverUri/$authorizationApi/$methodName',
        method: 'POST',
        params: params,
      ));
  ...
  ///Возвращает стандартные заголовки nsg-запроса: авторизация + версия протокола +
  ///хеш схемы + версия приложения.
  ...
  Map<String, String> getAuthorizationHeader() {
    ...
    // Версия приложения — в КАЖДОМ запросе. До этого сервер узнавал её только из
    // CheckVersion: один вызов за старт, который мы не дожидаемся (unawaited в
    // connect) и отправляем ещё до того, как вход заведёт новый токен. Сервер
    // хранил её копией в токене, и она отставала — у каждого узла свой кэш
    // токенов, который не перечитывается, пока токен активен. Человек на свежей
    // сборке получал «обновитесь до версии постарше» примерно на трёх запросах
    // из четырёх (NSG-SOFT/futbolista-tasks#312).
    //
    // Заголовок снимает вопрос в корне: версия приезжает вместе с запросом, о
    // котором и выносится решение. Сервер предпочитает её любой своей копии, а
    // если заголовка нет — работает по-старому.
    if (applicationVersion.isNotEmpty) {
      map['X-Nsg-App-Version'] = applicationVersion;
    }
    return map;
  }
