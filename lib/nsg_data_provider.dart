...
class NsgDataProvider {
  ...
  // 0.3.7. Поэтому проверка стала ОПЦИОНАЛЬНОЙ: по умолчанию выключена
  // ([verifyCertificates] = false, поведение как до 09.2026), приложение
  // включает её явно. Включённая проверка работает с двумя послаблениями ниже.

  ///Проверять TLS-сертификат сервера.
  ///
  ///`false` по умолчанию: принимается любой сертификат, как до 09.2026.
  ///Приложение, которому нужна защита от подмены (перехватчики, MITM), ставит
  ///`true` при инициализации — и тогда должно быть уверено, что корень его
  ///сервера есть на устройствах (или вшит в приложение через SecurityContext).
  ///
  ///```dart
  ///NsgDataProvider.verifyCertificates = true;
  ///```
  static bool verifyCertificates = false;
