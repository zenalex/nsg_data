...
class NsgDataItem {
  ...
  ///Прочитать объект из СВОЕЙ сериализации, а не из ответа сервера.
  ///
  ///Снимок, черновик, локальный кэш — всё, что писал сам клиент через [toJson].
  ///Отличие от [fromJson] ровно одно: строки табличной части сохраняют
  ///прочитанный `docState` вместо принудительного `saved`.
  ///
  ///**Зачем.** При `newTableLogic` удалённая строка не выкидывается из таблицы,
  ///а помечается: `docState = deleted`. [toJson] пишет у такой строки только
  ///первичный ключ — серверу для удаления больше ничего не нужно. Прочитав этот
  ///же json обычным [fromJson], мы получали бы ЖИВУЮ и ПУСТУЮ строку: пометка
  ///затёрта, полей нет. В заявке на матч это выглядело так, что убранный игрок
  ///возвращается в состав безымянной строкой, а его удаление не уезжает на
  ///сервер вовсе (NSG-SOFT/futbolista-tasks#1837, #1392).
  ///
  ///**Почему нельзя просто перестать затирать.** Принудительный `saved` держит
  ///удаление строк для ОТВЕТОВ СЕРВЕРА: своего состояния строк он в общем
  ///случае не присылает, а без `saved` строку потом не удалить — `removeRow`
  ///помечает удалённой только сохранённую. Хуже того, состояние сервер иногда
  ///присылает, и его `Marked = 2` — то же числовое значение, что у
  ///[NsgDataItemDocState.deleted]. То есть «сохранять docState, если он есть в
  ///json» показало бы помеченные на сервере строки удалёнными во всех
  ///табличных частях приложения. Поэтому режим включает только тот, кто ТОЧНО
  ///читает свою же запись.
  void restoreFromJson(Map<String, dynamic> json) {
    final previous = fieldValues.keepRowDocState;
    fieldValues.keepRowDocState = true;
    try {
      fromJson(json);
    } finally {
      fieldValues.keepRowDocState = previous;
    }
  }
  ...
  ///
  ///⚠️ Это СЕТЕВОЙ вызов, и с NSG-SOFT/futbolista-tasks#1921 он наконец делает то,
  ///что обещает: до этого ветка загрузки была мертва и метод молча возвращал
  ///пустышку, ничего не запрашивая. Вместе с работой у него появилась и цена,
  ///которой раньше не было, — поэтому:
  ///
  ///* не звать из `build()` и прочих синхронных мест: результат там не дождаться,
  ///  а `Future` без `await` при обрыве связи всплывёт в
  ///  `PlatformDispatcher.onError` необработанным;
  ///* не звать в цикле по списку: это N запросов по одному объекту. Список
  ///  дочитывается `referenceList` у запроса — одним разом и на сервере;
  ///* уместен там, где ссылка нужна редко, точечно и вне списочного запроса.
  ///
  ///Для «возьму, если уже загружено» есть синхронный `getReferent` — он в сеть
  ///не ходит и не ждёт.
  ...
  NsgValidateResult validateFieldValues({NsgBaseController? controller}) {
    ...
    for (var fieldName in fieldList.fields.keys) {
      if (isFieldRequired(fieldName)) {
        ...
        final hasExplicitValue = fieldValues.fields.containsKey(fieldName) && !fieldValues.emptyFields.contains(fieldName);
        final isMissing = field is NsgDataEnumReferenceField ? !hasExplicitValue : !hasExplicitValue || fieldValues.fields[fieldName] == field.defaultValue;
        if (isMissing) {
